...
`"point" | "line" | "bar" | "area" | "rule" | "arc" | "text"`로 materialize한다. 이 결정은 context에만 두지 않고
...
Text annotation은 current 또는 unique compatible point/bar/rule layer를 semantic `source`로 저장한다.
...
만들고 rule은 final endpoint에 붙는다. Text 내용은 scale 없는 `encoding.text` field/datum assignment이며,
