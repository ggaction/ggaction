...
이 문서만 받은 구현자는 다음 순서로 시작한다. **현재는 상세 계획이며 제품 코드는 구현되지 않았다.**
...
6. [PROPOSED_TYPES.d.ts](PROPOSED_TYPES.d.ts): 새/변경 옵션의 검토용 타입. 기존 types와 자동 병합되지 않는다.
...
실제 API의 진실은 현재 source/types/current contract다. 이 폴더는 미래 구현 제안이다.
이 폴더 내부에서 상세 의미는 IMPLEMENTATION_SPEC/INSPECTION_SPEC와 PROPOSED_TYPES가 소유하고,
FEATURES는 설명용 요약, ROADMAP은 순서, DECISIONS는 승인·호환성을 소유한다.
타입과 동작 문장이 충돌하면 구현자가 임의 선택하지 말고 문서 오류를 먼저 정정한다.
구현 승인을 받기 전에도 이번 요청처럼 구체적인 설계·fixture를 작성하고 검증할 수 있다.
...
- 구현을 시작할 권한과 D01–D14 중 해당 결정의 승인 근거를 확인한다. 이번 상세화 요청은 runtime
  구현 승인이 아니다. 승인된 결정을 반복해서 묻지 않으며 제외된 D08에는 Gate를 만들지 않는다.
