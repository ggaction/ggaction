...
2026-09-13 상세화에서 확정한 정정 기록이다. 기존 user-selected 25개 범위를 유지한 채 모든 항목이 승인·구현됐으며, 이 표는 초안의 빈칸과 충돌을 어떤 이유로 고쳤는지 보존한다. 정확한 현재 동작은 각 feature의 구현 고정 명세, `agent_docs/contract/current`, 제품 타입과 source가 소유한다.
