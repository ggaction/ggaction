...
원래 감사 번호: **25**. Primary owner: **Phase 11**. 상태: **Implemented-primary**.
아래 세부 API·수치 정책은 제품 checkpoint `c29f496c`에서 구현·검증됐다. Full-only
`removeData`, `removeScale`, `removeCoordinate`, typed reference registry, 기존 owner 수명주기
연결과 설치 패키지 소비자까지 같은 checkpoint가 소유한다.
...
- [x] concat은 cross-child namespace 때문에 명시적으로 거부하고 unit/facet parent 범위를 검증했다.
- [x] Phase 11 승인과 제품·검증 근거를 `phase11/STEP1.md`에 기록했다.
