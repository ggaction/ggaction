...
사용자는 감사 번호 25개와 Phase 0–12 Gate를 모두 승인했다. Phase 0–7 primary 구현이 완료됐고, 현재 작업은 Phase 8의 R37 exact sampled legend values다. 승인된 공개 API·옵션·수식은 구현 계약이지만, 실제 source·tests·types·Current contract 증거가 없는 기능을 구현 완료로 간주하지 않는다.
...
현재 구현은 [WP8.1 R37 exact legend values](DETAILED_IMPLEMENTATION_WORK_PACKAGES.md#wp81--r37-exact-legend-values)다. Phase 7의 R31 `73e3d53e`, R32 `ee3f3b02`, R33 `95968031`, R36 `d7136174`와 통합 `5832228c`를 다시 구현하지 않는다. size/opacity/strokeWidth 연속 범례의 requested sample mode와 exact value validation, scale edit preflight, guide/Canvas/theme replay를 R37 계약 순서대로 구현한다.
