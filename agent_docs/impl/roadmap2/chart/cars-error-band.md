...
Cars dataset의 연도별 평균 `Acceleration`과 95% 신뢰구간을 `Origin`별 area band로 표현한다. 이 차트는
vertical/horizontal ranged area, 통계/명시 interval, optional boundary line, curve 공유와
`createErrorBand` composite hierarchy의 canonical oracle이다. Gapminder variant는 동일 계약을 다른 데이터와
반대 orientation에서 검증한다.

## Canonical target

- Data: finite `Acceleration`, valid `Year`, non-empty `Origin`을 가진 cars rows
- Independent position: x = temporal `Year`
- Interval: y = `mean(Acceleration)` with two-sided 95% Student-t confidence interval
- Series: `Origin`, source first-appearance order
- Mark: Origin별 closed area path; boundary lines는 기본적으로 없음
- Fill: `Origin` categorical color through existing `encodeColor`
...
- Title: `Acceleration Trend by Origin`; subtitle: `Mean and 95% confidence interval`
...
### Cars canonical vertical band

Cars는 ISO-like temporal strings, three Origin series와 grouped vertical statistical derivation을 검증한다.
통계 reference는 production transform을 import하지 않고 sample variance와 Student-t critical value로 계산한다.

### Gapminder horizontal range

Gapminder는 numeric four-digit `year`를 temporal로 normalize하고, 각 year의 `life_expect` 평균과 95% CI를
horizontal x/x2 band로 표현한다.
...
chart()
  ...
  .createTitle({
    ...
    subtitle: "Mean and 95% confidence interval across countries"
  });
```

이 variant는 Cars chart의 field, row count, temporal representation과 orientation을 모두 바꿔서 구현이
...
| --- | --- | --- | --- |
| A | `cars-vertical` | cars | grouped statistical y/y2 area, default no boundaries |
| B | `gapminder-horizontal` | gapminder | numeric temporal y, statistical x/x2 area, boundaries |
| C | `cars-curved-boundaries` | cars | curve inheritance/override and custom boundary styles |

각 variant는 raw primitive를 먼저 만들고 승인 후 public program을 구현한다. 저장 경로는
`.artifacts/test/png/roadmap2/cars-error-band/<variant>/{primitive,user-facing}.png`이며 manifest가 exact target
