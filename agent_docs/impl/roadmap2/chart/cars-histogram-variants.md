...
| `bin-step` | `encodeHistogram({ field: "Displacement", binStep: 60 })` | zero-anchored exact-width bins |
...
- 선택한 mode와 authored 값은 semantic state에 저장한다. Auto-resolved domain/boundaries와 concrete rect는
  resolved scale 및 graphic state에 남고 Renderer는 bin policy를 해석하지 않는다.
