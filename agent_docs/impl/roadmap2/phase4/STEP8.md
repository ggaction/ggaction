# Roadmap 2 — Phase 4 Step 8: Regression Variation Primitives

## 목표

Quadratic polynomial, LOESS line-only와 prediction interval의 independent numeric/graphic targets를 고정한다.

## 진행 상태

- [x] Polynomial coefficients/fitted-row fixture
- [x] LOESS neighbor/fitted-row fixture
- [x] Mean/prediction interval fixture
- [x] Three raw primitive chart variants
- [x] Line-only LOESS layer/trace target
- [x] Expanded target call-chain metadata
- [x] Browser와 2× primitive PNG 생성
- [x] Gate D 사용자 visual confirmation
- [x] Feedback, status, commit와 push

세 target은 수정 없이 승인되었다.

## 완료 조건

세 model/interval targets와 visible paths are approved before public implementation.
