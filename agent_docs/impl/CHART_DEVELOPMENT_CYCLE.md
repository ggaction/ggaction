...
새 Phase를 시작할 때 사용자는 최소한 다음을 제시한다.
...
이 정보와 Phase의 `GOAL.md`가 확정되면 agent는 아래 사이클을 순서대로 자율
진행할 수 있다. Public API, 저장 schema, 핵심 architecture를 바꿔야 하거나 목표가
여러 방식으로 해석될 때만 사용자와 다시 합의한다.

## Phase 산출물

각 Phase는 `agent_docs/impl/phaseN/` 아래에 다음 문서를 둔다.

- `GOAL.md`: 목표 차트와 최종 구현 계약
- `STEP1.md`, `STEP2.md`, ...: 구현 순서와 진행 상태

완료된 Phase는 다음 실행 가능한 결과를 남긴다.
...
구현 전에 `GOAL.md`를 먼저 작성한다. 이 문서는 단순한 아이디어가 아니라 Phase의
완료 조건이다.
...
의존성 순서대로 `STEPn.md`를 작성한다. 각 문서는 목표 바로 아래에 `진행 상태`
체크리스트를 둔다.
...
이번 단계에서 완성할 observable behavior.
...
하나의 STEP은 독립적으로 검증 가능한 의미 단위여야 한다. 파일 하나나 작은 helper
하나를 STEP으로 만들지 않고, 한 번에 전체 Phase를 담지도 않는다.
...
9. STEP 진행 상태를 갱신하고 하나의 conceptual commit으로 push한다.
...
하위 action이 준비되면 `GOAL.md`의 최종 API chain으로 public example을 작성한다.
...
Phase 완료 전에 다음을 수행한다.

- `GOAL.md`와 모든 STEP의 진행 상태를 완료로 갱신한다.
- Public action reference, 관련 API page, tutorial, `docs/llms.txt`를 갱신한다.
- Phase 진행 중 사용한 intermediate program과 snapshot test를 제거한다.
- 차트별 public example과 primitive baseline만 대표 프로그램으로 남긴다.
- Phase/STEP 번호가 들어간 test 이름을 기능 중심 이름으로 바꾼다.
...
- [ ] Intermediate Phase artifact가 정리되었다.
