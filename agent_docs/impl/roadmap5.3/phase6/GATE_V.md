...
이 문서는 새 비용 승인 제안이다. Gate S, T와 U approval은 재사용하지 않는다. 사용자가 이 Gate를 별도로 승인해야만 exact
local approval artifact를 만들고 credential을 읽으며 OpenAI Responses API를 호출할 수 있다. 현재까지 Gate V를 위한
credential read, external model call과 spend는 **0 / 0 / $0**이다.
