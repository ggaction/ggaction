...
import { visualVariants as series, inputHashes as seriesHashes } from "../../../../test/gates/series-identity/manifest.js";
...
for (const variant of [...series, ...temporal]) {
  ...
  const sourcePaths = ["fixture.js", "reference-values.js", "primitive.program.js", "manifest.js"]
    .map(file => `test/gates/${variant.chart}/${file}`);
  ...
  results.push({ chart: variant.chart, variant: variant.variant,
    ...
    publicImplementation: "pending-visual-approval" });
  cards.push(`<article><h2>${escape(variant.title)}</h2><img alt="${escape(variant.title)}" src="data:image/png;base64,${png.toString("base64")}"><details><summary>승인할 정확한 public action chain</summary><pre><code>${escape(variant.callChain)}</code></pre></details><p>primitive source: <code>test/gates/${variant.chart}/primitive.program.js</code><br>input SHA-256: <code>${inputSha256}</code></p></article>`);
}
