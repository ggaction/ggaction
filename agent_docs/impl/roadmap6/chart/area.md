...
Simple area의 value와 constant baseline, ribbon의 두 endpoint, stack의 누적 endpoint를 구분하여 실제 채워진 영역을 만든다.

- 연결 항목: F05, D03, D14, B02.
...
const values = [{ time: 1, value: 2, low: 1, high: 3 }, { time: 2, value: 4, low: 2, high: 6 }, { time: 3, value: 3, low: 1, high: 5 }];
const base = chart()
  .createCanvas({ width: 1000, height: 700, margin: 150 })
  .createData({ id: 'data', values });

const area = base.createAreaPlot({ x: 'time', y: 'value', baseline: 0 });
const ribbon = base.createAreaPlot({
  x: 'time', y: { lower: 'low', upper: 'high' }
});
~~~
...
| 결정 | 권장 계약 | 이유 |
| --- | --- | --- |
| Simple | 기본 baseline=0을 data-space 의미로 기록한다. | x/y만 있는 빈 Area와 구분한다. |
| Range | lower/upper fields를 한 번에 preflight하고 기존 range owner를 재사용한다. | 반쪽 range가 그래픽으로 노출되지 않는다. |
| Stack | Group/stack/fill/centered는 Phase 4 layout owner에 명시한다. Negative와 normalized 분모를 고정한다. | 색과 무관하게 배치를 제어한다. |
| Scales | Baseline을 automatic domain에 포함한다. Log에서 0 baseline은 거부하고 명시적 valid baseline을 요구한다. | Log에 임의 epsilon을 끼워 넣지 않는다. |
| Missing | Path break, invalid endpoint, lower>upper 처리는 명시적으로 결정한다. | 누락 데이터를 임의 연결하여 영역을 왜곡하지 않는다. |
...
createAreaPlot
├─ createAreaMark
├─ encodeX / encodeY (role-aware pair)
├─ atomic baseline or range assignment (proposed lower action)
│  ├─ data-space endpoint validation
│  └─ shared scale + area materialization
├─ explicit group / layout assignment?
├─ encodeColor?
└─ createGuides?
~~~
...
**semanticSpec:** Field endpoints 또는 constant baseline, orientation/group/stack policy, source grain과 scale consumers를 저장한다. 가짜 zero source field를 추가하지 않는다.

**graphicSpec:** 두 경계와 closure를 포함한 ordinary area commands를 concrete 좌표로 저장한다. Incomplete lower intent에는 placeholder path가 없다.
...
Baseline/range/layout은 atomic lower assignment, appearance는 editAreaMark, position/scale는 기존 editor가 소유한다. 모드 전환 때 stale y2/x2/offset/stack policy를 정리한다.
...
- Simple positive와 signed data, vertical/horizontal, baseline 0/명시값.
- Ranged ribbon, missing middle point, swapped endpoint failure policy.
- Group→stack→group, normalized stack, supported centered area.
- Linear/log baseline compatibility, shared guide/selection edit.
...
관측점마다 baseline 또는 lower/upper의 data-to-screen mapping을 독립 계산한다. Shortest valid call의 item/path가 존재하며 빈 data는 별도 정책으로 확인한다.
...
Arbitrary callback baseline, new renderer path type, 모든 interpolation/stack 조합의 자동 지원은 포함하지 않는다.
