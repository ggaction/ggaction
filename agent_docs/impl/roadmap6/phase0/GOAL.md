...
상태: planned. F01–F19의 범위 선택을 기록했으며 구현·Gate 승인은 아직 없다.

기준 제품과 감사 증거를 고정하고, 실행 대상 47개 항목을 빠짐없이 작업 단위에 연결한다. 이 단계는 계획과 결정 기록이며 제품 구현을 시작하지 않는다.
...
- 연결: 로드맵 공통 관리.
- 작업: H0–H4, exposure, lifecycle을 분리한다. 범위에 포함된 B01–B08, D01–D20, F01–F19에 primary owner, dependent phases와 acceptance를 부여한다.
- 완료 조건: 47개 ID 중 누락·중복 primary owner 0; 전체 액션 전수표 링크가 존재한다.
...
- 상대 규모: M. 시간 약속이 아닌 변경 구조 비교다.
- 연결: 로드맵 공통 관리.
- 작업: 수치·계층·편집·render·types·MCP·package matrix와 선택한 19개 액션군의 납품 순서를 작성한다.
