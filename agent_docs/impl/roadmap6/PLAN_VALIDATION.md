...
미래 구현 단계의 통과·Gate 승인·버그 수정 완료를 뜻하지 않는다.

## 변경 범위

- Roadmap 6: 12개 Phase, 46개 작업 묶음, 13개 차트군 계약.
- B01–B08, D01–D20, F01–F19의 47개 대상 항목에 primary 작업과 acceptance를 부여했다.
- 2026-09-05 사용자 선택에 따라 F20과 관련 연구 작업·문서·완료 조건을 제외했다.
- 공통 결정 16개, 하위 액션군, compatibility/migration, 검증 matrix를 작성했다.
...
| 47개 finding ID 전체 집합·primary owner·related work 대조 | 누락·중복 primary 0 |
| 범위 제외 검증 | F01–F19만 실행 대상, 제외한 F20 작업·문서·완료 조건 없음 |
| Phase dependency, work package ID, Gate state 점검 | 12 phases, 46 work packages, 모든 Gate planned |
...
범위 조정 뒤 contract suite(안의 navigation 7개 포함), 원장·의존 관계·링크·제외 범위를 다시 검증했다.
Audit replay와 type probe는 최초 계획 commit 0f048bb45e52d1348077c1b9e31e16bd7d1e6b14의 검증 기록이며
이번 문서 범위 변경에서 재실행하지 않았다. 보존된 원본 관측 파일은 변경하지 않았다.

API·MCP probe의 observed rejection은 기준 오류·제한을 재현한 것이다.
