...
- These target-resolution rules govern the library's runtime API. Never silently select the first eligible resource. Explicit targets win; otherwise use the current unique eligible resource, then one unique eligible resource, and reject ambiguity.
...
- By default, a create action requires a missing resource and an edit requires an existing resource and rejects an empty edit. Preserve explicitly documented lifecycle exceptions, including idempotent creation and revision-producing creation. Do not infer new exceptions from naming alone or remove accepted current behavior to satisfy this general rule.
...
- Context is private transient convenience, not semantic state and not an action. Update it immutably. Persist semantic decisions in `semanticSpec`, concrete rendered results in `graphicSpec`, and replayable non-semantic authoring intent in the canonical `materializationConfigs` owner defined by the architecture. Do not duplicate one intent across independent state owners.
