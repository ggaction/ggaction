...
| Aggregation | Immutable line mean aggregation; derived histogram bin totals for y scale resolution |
...
Automatic histogram rectangle materialization is not yet implemented. Polar
guide graphics, additional mark types, public transforms, facets, additional
legend types or positions, and program composition are also not implemented in
the current release. Additional title positions, automatic text wrapping, and
text measurement are unsupported. These features are not part of the current
API reference.
