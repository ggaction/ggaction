...
const resultsRoot = path.join(paidSmokeRootV5, "results");
if (process.argv.includes("--dry-run")) {
  process.stdout.write(`${JSON.stringify(await runPaidSmokeDryRunV5(), null, 2)}\n`);
} else {
  ...
  const plan = await assertPaidSmokeAuthorizedV5();
  ...
  try {
    const result = await runPaidSmokeV5({
      plan,
      apiKey,
