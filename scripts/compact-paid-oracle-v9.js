...
import { docsFallbackResources, SEARCH_TOOL_NAME } from "../src/mcp/adapter.js";
import { paidSmokeRouteV6 } from "./compact-paid-smoke-v6.js";
import { root } from "./compact-runtime-closure-v2.js";

export const paidComparisonRootV9 = path.join(
  root,
  "evaluation",
  "compact-authoring-paid-comparison-v9"
);
export const routeOracleFileV9 = path.join(paidComparisonRootV9, "ROUTE_ORACLE.json");
...
async function sourceTasks(oracle) {
  ...
  if (sha256(finalOracleBytes) !== oracle.sourceFinalOracleSha256) {
    throw new Error("Paid comparison v9 source final oracle drifted.");
  }
  ...
  if (finalOracle.productCandidateCommit !== oracle.productCandidateCommit) {
    throw new Error("Paid comparison v9 product candidate drifted from final v3.");
  }
  ...
  return oracle.tasks.map(selection => {
    ...
    if (!task) throw new Error(`Unknown paid comparison v9 task: ${selection.id}`);
    ...
    for (const [actual, expected, label] of checks) {
      if (!same(actual, expected)) {
        throw new Error(`${selection.id} v9 ${label} drifted: ${JSON.stringify(actual)}`);
      }
    }
    ...
    if (!same(expectedDRoute, selection.expectedDRoute)) {
      throw new Error(`${selection.id} v9 D route drifted.`);
    }
    return Object.freeze({ ...task, dataset, expectedDRoute: selection.expectedDRoute });
  });
}

export async function loadRouteOracleV9() {
  const bytes = await readFile(routeOracleFileV9);
  const oracle = JSON.parse(bytes);
  if (
    oracle.schemaVersion !== 1 ||
    oracle.id !== "compact-authoring-paid-comparison-route-oracle-v9" ||
    oracle.sourceFinalCorpus !== "compact-authoring-final-v3" ||
    oracle.packetSchemaVersion !== 3 ||
    oracle.tasks.length !== 24 ||
    oracle.conditions.length !== 4 ||
    new Set(oracle.tasks.map(task => task.id)).size !== 24
  ) {
    throw new Error("Paid comparison v9 route oracle identity is invalid.");
  }
...
export function threeModelRunOrderV9(
  tasks,
  ...
  if (
    ...
    new Set(modelIds).size !== 3 ||
    new Set(conditionIds).size !== 4 ||
    !Number.isInteger(repetitions) ||
    repetitions < 1 ||
    !tasks.every(task => typeof task?.id === "string" && task.id.length > 0)
  ) {
    throw new Error("Paid comparison v9 requires three models, four conditions, tasks, and repetitions.");
  }
...
export function parseThreeModelRunV9(run) {
  const match = /^(.+):r([1-9][0-9]*):(gpt-[^:]+):([A-D])$/u.exec(run);
  if (!match) throw new Error(`Invalid paid comparison v9 run: ${run}`);
  return {
    taskId: match[1],
...
export function modelCallEnvelopeV9(tasks, modelIds, conditionIds, repetitions, submissions) {
