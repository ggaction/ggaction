...
const resultsRoot = path.join(paidSmokeRootV4, "results");
if (process.argv.includes("--dry-run")) {
  process.stdout.write(`${JSON.stringify(await runPaidSmokeDryRunV4(), null, 2)}\n`);
} else {
  ...
  const plan = await assertPaidSmokeAuthorizedV4();
  ...
  try {
    const result = await runPaidSmokeV4({
      plan,
      apiKey,
