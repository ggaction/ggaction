...
export function forcedFunctionToolChoiceV4(name) {
...
function baseRequest({ plan, adapter, submitTool, input, maximumOutputTokens, forcedTool }) {
  ...
  return {
    ...
    tool_choice: forcedFunctionToolChoiceV4(forcedTool),
    tools,
    instructions: [
...
function responseMetadata(response, request) {
  ...
  return {
    identity: {
      requestedModel: typeof request?.model === "string" ? request.model : null,
      returnedModel: typeof response?.model === "string" ? response.model : null,
      requestedServiceTier: typeof request?.service_tier === "string"
        ? request.service_tier
        : null,
      returnedServiceTier: typeof response?.service_tier === "string"
...
export async function runBoundedToolStateMachineV4({
  plan,
  task,
  condition,
  apiKey,
  ...
  try {
    for (let callIndex = 0; callIndex < maximumCallsForRoute; callIndex += 1) {
      ...
      for (let retryIndex = 0; ; retryIndex += 1) {
        ...
        try {
          ...
          responseTraceEntry = {
            ...
            provider: responseMetadata(response, request),
      ...
      const identity = responseTraceEntry.provider.identity;
      if (identity.returnedModel !== identity.requestedModel) {
        throw new Error(
          `provider-model-identity-mismatch:requested-${identity.requestedModel ?? "missing"}` +
          `:returned-${identity.returnedModel ?? "missing"}`
        );
      }
      if (identity.returnedServiceTier !== identity.requestedServiceTier) {
        throw new Error(
          `provider-service-tier-mismatch:requested-${identity.requestedServiceTier ?? "missing"}` +
          `:returned-${identity.returnedServiceTier ?? "missing"}`
        );
      }
      if (
