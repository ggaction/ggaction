...
import {
  assertPaidComparisonAuthorizedV10,
  loadApiKey,
  paidComparisonRootV10,
  preflightPaidComparisonToolsV10,
  runPaidComparisonDryRunV10,
  runPaidComparisonV10
...
const resultsRoot = path.join(paidComparisonRootV10, "results");
if (process.argv.includes("--dry-run")) {
  process.stdout.write(`${JSON.stringify(await runPaidComparisonDryRunV10(), null, 2)}\n`);
} else {
  ...
  try {
    const result = await runPaidComparisonV10({
      plan,
      apiKey,
