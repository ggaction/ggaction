...
const resultsRoot = path.join(paidSmokeRootV6, "results");
if (process.argv.includes("--dry-run")) {
  process.stdout.write(`${JSON.stringify(await runPaidSmokeDryRunV6(), null, 2)}\n`);
} else {
  ...
  const plan = await assertPaidSmokeAuthorizedV6();
  ...
  try {
    const result = await runPaidSmokeV6({
      plan,
      apiKey,
