...
import {
  assertPaidSmokeAuthorizedV7,
  loadApiKey,
  paidSmokeRootV7,
  preflightPaidSmokeToolsV7,
  runPaidSmokeDryRunV7,
  runPaidSmokeV7
} from "./compact-paid-smoke-v7.js";
...
const resultsRoot = path.join(paidSmokeRootV7, "results");
if (process.argv.includes("--dry-run")) {
  process.stdout.write(`${JSON.stringify(await runPaidSmokeDryRunV7(), null, 2)}\n`);
} else {
  ...
  const plan = await assertPaidSmokeAuthorizedV7();
  ...
  try {
    const result = await runPaidSmokeV7({
      plan,
      apiKey,
