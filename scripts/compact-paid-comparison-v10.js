...
import { runBoundedToolStateMachineV4 } from "./compact-paid-state-machine-v4.js";
...
export { loadApiKey, root };
export { loadRouteOracleV9, modelCallEnvelopeV9, parseThreeModelRunV9, threeModelRunOrderV9 };

export const paidComparisonRootV10 = path.join(
  root,
  "evaluation",
  "compact-authoring-paid-comparison-v10"
);
export const paidComparisonPlanFileV10 = path.join(paidComparisonRootV10, "PLAN.json");
const paidComparisonGateFileV10 = path.join(
  root,
  "agent_docs",
  "impl",
  "roadmap5.4",
...
export function planForModelV10(plan, modelId) {
  const model = modelConfig(plan, modelId);
  return {
    ...plan,
    api: { ...plan.api, model: model.requestModel },
...
function assertPlanShapeV10(plan) {
  if (
    plan.schemaVersion !== 1 ||
    plan.id !== "compact-authoring-paid-comparison-v10" ||
    plan.requiredGate !== "R54-P6-B"
  ) {
    throw new Error("Paid comparison v10 plan identity is invalid.");
  }
  ...
  if (
    ...
    !plan.models.every((model, index) => model.requestModel === expectedRequestModels[index])
  ) {
    throw new Error("Paid comparison v10 models are invalid.");
  }
  ...
  for (const model of plan.models) {
    ...
    if (JSON.stringify(actual) !== JSON.stringify(expectedPricing[model.id])) {
      throw new Error(`Paid comparison v10 pricing drifted for ${model.id}.`);
    }
  }
  if (plan.runOrder.length !== 576 || new Set(plan.runOrder).size !== 576) {
    throw new Error("Paid comparison v10 run order must contain 576 unique cells.");
  }
  if (
    ...
    throw new Error("Paid comparison v10 evaluator files and product trees must be frozen.");
  }
  if (
    ...
    throw new Error("Paid comparison v10 limits are invalid.");
  }
  ...
  if (
    ...
    throw new Error("Paid comparison v10 cost projection does not match its rolling cap.");
  }
}
...
function commitFile(commit, relative, label) {
  try {
    ...
    throw new Error(`Paid comparison v10 ${label} is unavailable: ${relative}`);
  }
}
...
function commitTree(commit, relative) {
  try {
    ...
    throw new Error(`Paid comparison v10 product tree is unavailable: ${relative}`);
  }
}
...
function assertAncestor(commit, label) {
  try {
    ...
    throw new Error(`Paid comparison v10 ${label} is not an ancestor of the current HEAD.`);
  }
}
...
function assertCurrentProduct(candidate) {
  try {
    ...
    throw new Error("Paid comparison v10 current product differs from its frozen candidate.");
  }
}

export async function loadPaidComparisonPlanV10() {
  const [planBytes, oracle] = await Promise.all([
    readFile(paidComparisonPlanFileV10),
    loadRouteOracleV9()
  ]);
  const plan = JSON.parse(planBytes);
  assertPlanShapeV10(plan);
  if (plan.routeOracleSha256 !== oracle.oracleSha256) {
    throw new Error("Paid comparison v10 route oracle hash drifted.");
  }
  ...
  if (JSON.stringify(plan.runOrder) !== JSON.stringify(expectedRunOrder)) {
    throw new Error("Paid comparison v10 run order is not the frozen 12-cell Latin square.");
  }
  ...
  if (
    ...
    throw new Error("Paid comparison v10 model-call envelope drifted from the frozen routes.");
  }
  for (const [relative, expected] of Object.entries(plan.evaluatorSourceFiles)) {
    ...
    if (sha256(frozen) !== expected) {
      throw new Error(`Paid comparison v10 evaluator source hash drifted: ${relative}`);
    }
    const current = await readFile(path.join(root, relative));
    if (sha256(current) !== expected) {
      throw new Error(`Paid comparison v10 current evaluator source drifted: ${relative}`);
    }
  }
  for (const [relative, expected] of Object.entries(plan.productSourceTrees)) {
    if (commitTree(plan.productCandidateCommit, relative) !== expected) {
      throw new Error(`Paid comparison v10 product tree hash drifted: ${relative}`);
    }
  }
...
export async function assertPaidComparisonAuthorizedV10(plan) {
  const approved = plan ?? await loadPaidComparisonPlanV10();
  const gate = await readFile(paidComparisonGateFileV10, "utf8");
...
function routeFailuresV10(condition, task, snapshot) {
  const failures = [];
...
export async function preflightPaidComparisonToolsV10() {
  return preflightPaidComparisonToolsV8();
}

export async function runPaidComparisonTaskV10(options) {
  const model = modelConfig(options.plan, options.model);
  const createResponse = options.createResponse ?? createOpenAIResponseV2;
  const taskPlan = planForModelV10(options.plan, options.model);
  return runBoundedToolStateMachineV4({
    ...options,
    plan: taskPlan,
    ...
    validateRoute: routeFailuresV10,
...
export async function runPaidComparisonDryRunV10({
  artifactRoot = path.join(root, ".artifacts", "evaluation", "compact-paid-comparison-v10-dry")
} = {}) {
  ...
  for (const task of oracle.tasks) {
    for (const condition of oracle.conditions.map(entry => entry.id)) {
      ...
      try {
        ...
        const failures = routeFailuresV10(condition, task, adapter.snapshot());
...
export async function runPaidComparisonMatrixV10(options) {
...
export async function runPaidComparisonV10({
  plan: suppliedPlan,
  apiKey,
  createResponse,
  artifactRoot,
  ...
  const plan = await assertPaidComparisonAuthorizedV10(
    suppliedPlan ?? await loadPaidComparisonPlanV10()
  );
  await preflightPaidComparisonToolsV10();
  return runPaidComparisonMatrixV10({
    plan,
    apiKey,
    createResponse,
    artifactRoot,
