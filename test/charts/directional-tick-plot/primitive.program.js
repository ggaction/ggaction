...
function panelBase({ title, subtitle }) {
  return chart()
    ...
    .createGraphics({ id: "anchors", parent: "plot-main", type: "circle", length: ANCHORS.length })
    ...
    .createGraphics({ id: "directionLabels", parent: "plot-main", type: "text", length: LABELS.length })
...
function addTicks(program, { id, values, stroke }) {
  return program
    .createGraphics({ id, parent: "plot-main", type: "line", length: values.length })
...
export function createDirectionalTickPointPrimitives() {
  ...
  const directionalPoints = panelBase({
    ...
    .createGraphics({ id: "points", parent: "plot-main", type: "collection" })
    .editGraphics({
      target: "points",
      property: "items",
      value: DIRECTIONAL_TRIANGLES.map(commands => ({
        type: "path",
        properties: {
          commands,
          fill: "#f97316",
...
export function createHorsepowerRugPrimitives(cars) {
  ...
  return chart()
    ...
    .editGraphics({ target: "canvas", property: "background", value: "#ffffff" })
    .createGraphics({ id: "plot-main", parent: "canvas", type: "collection" })
    .createGraphics({ id: "grid", parent: "plot-main", type: "line", length: values.axisX.length })
    ...
    .createGraphics({ id: "ticks", parent: "plot-main", type: "line", length: values.rows.length })
    ...
    .createGraphics({ id: "axisLabels", parent: "plot-main", type: "text", length: values.labels.length })
