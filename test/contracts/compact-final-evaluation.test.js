...
test("preserves the second frozen final attempt and its three failures", async () => {
  const [corpusBytes, datasetsBytes, oracleBytes, resultBytes] = await Promise.all([
    ...
    readFile(path.join(secondDirectory, "RESULT.json"))
  ]);
  const corpus = JSON.parse(corpusBytes);
  const oracle = JSON.parse(oracleBytes);
  const result = JSON.parse(resultBytes);
  ...
  assert.equal(sha256(resultBytes), "b9b24b7aa20b850aa1c93eabfbb23c2aa9809cb4ce27f11e2811e5dd9b2e01ba");
  ...
  assert.deepEqual(
    result.evaluations.filter(entry => !entry.passed).map(entry => entry.task),
    ["final2-03-bars-png", "final2-12-rule-canvas", "final2-18-raw-bars-canvas"]
  );
  assert.equal(result.externalCalls, 0);
  assert.equal(result.credentialReads, 0);
  assert.equal(result.spendUsd, 0);
