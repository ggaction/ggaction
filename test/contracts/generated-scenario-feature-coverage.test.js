...
test("derives a bounded public option inventory without runtime prototype paths", async () => {
  ...
  assert.equal(inventory.counts.publicActions, 222);
  assert.equal(inventory.counts.topLevelOptionPaths, 1637);
  assert.equal(inventory.counts.nestedOptionPaths, 8248);
  assert.equal(inventory.counts.optionPaths, 9885);
  assert.equal(inventory.counts.requiredOptionPaths, 8630);
  assert.equal(inventory.counts.excludedOptionPaths, 1255);
  assert.equal(inventory.counts.topLevelCategoricalPaths, 457);
  assert.equal(inventory.counts.topLevelLiteralValues, 2127);
  assert.equal(inventory.counts.literalFamilies, 120);
  assert.equal(inventory.counts.pathLiteralRequirements, 5168);
  ...
  assert.equal(ledger.requirements.length, 14261);
