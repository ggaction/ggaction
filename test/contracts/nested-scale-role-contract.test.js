...
function sizeScale(type) {
  return {
    type,
    ...(type === "log" ? { domain: [1, 20], base: 2 } : {}),
    ...(type === "pow" ? { exponent: 2 } : {}),
    ...(type === "quantize" ? { domain: [1, 20], range: [4, 16] } : {}),
    ...(type === "quantile" ? { range: [4, 16] } : {}),
    ...(type === "threshold" ? { domain: [4], range: [4, 16] } : {})
  };
}
...
function buildScaleWitness(action, path, type) {
  switch (action) {
    ...
    case "createTextPlot":
      return source().createTextPlot({
        x: path.startsWith("x.") ? positionChannel(type, "x") : "x",
        y: path.startsWith("y.") ? positionChannel(type, "y") : "y",
        text: "category",
        ...(path.startsWith("color.") ? { color: colorChannel(type) } : {}),
        guides: false
      });
    ...
    case "createAreaPlot":
      ...
      if (["time", "point", "band"].includes(type)) return source().createAreaPlot(path.startsWith("x.")
        ? { x: positionChannel(type), y: "value", guides: false }
    ...
    case "createPiePlot":
      return source().createPiePlot({
        category: path === "category.scale.type" ? { field: "category", scale: { type } } : "category",
        ...(path === "color.scale.type" ? { color: colorChannel(type) } : {}), ...(path === "radius.scale.type" ? { radius: { aggregate: "count", scale: { ...positionScale(type), range: [20,80] } } } : {}), guides: false
      });
...
test("derives only role-reachable nested scale type paths", async () => {
  ...
  assert.equal(scaleTypes.length, 150);
  assert.equal(scaleTypes.reduce((sum, option) => sum + option.values.length, 0), 665);
  ...
  assert.ok(options.get("option-path:encodeColor.scale.type").values.includes("string:symlog"));
