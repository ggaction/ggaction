...
const cdnUrl = "https://cdn.jsdelivr.net/npm/ggaction@0.0.11/+esm";
...
test("keeps the Quarto example pinned, responsive, and accessible", () => {
  ...
  assert.match(cdnCheck, /const expectedVersion = "0\.0\.11"/);
...
test("executes the example program against the package source", async () => {
  ...
  assert.equal(example.GGActionVersion, "0.0.11");
